...
// hiking
const url = `https://www.komoot.com/api/v007/discover_tours/?srid=4326&format=simple&fields=timeline&timeline_highlights_fields=images,start_point&limit=100&max_distance=500000&sport=hike&surface=prefer_unpaved&hl=en&usersetting.creator.username=3437280258324`
  ...
  try {
    ...
    for (let page = 0; page < totalPages; page++) {
      ...
      const dbSpots = await prisma.spot.findMany({
        ...
        where: { type: "HIKING_TRAIL", komootId: { in: newSpots?.map((s: any) => s.id) } },
      })
...
async function main() {
  try {
    ...
    const dbIds = await prisma.spot.findMany({select: {komootId: true, id: true}, where: {deletedAt: null, komootId: {not: null}, type: SpotType.HIKING_TRAIL}})
