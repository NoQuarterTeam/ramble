...
export function useS3Upload(): [(file: File) => Promise<UploadFile>, { isLoading: boolean }] {
  ...
  async function upload(file: File) {
    try {
      ...
      setIsLoading(false)
      return { fileUrl: signedUrl, fileKey: key, fileName: file.name, fileType: file.type || null }
    } catch (error) {
      setIsLoading(false)
      throw error
    }
